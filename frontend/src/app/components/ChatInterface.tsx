...
import { ChangePreview } from "@/app/components/ChangePreview";
import type {
  TodoItem,
  ToolCall,
  ActionRequest,
...
export const ChatInterface = React.memo<ChatInterfaceProps>(({ assistant }) => {
  ...
  // Track locally rejected change IDs so they disappear instantly
  const [rejectedIds, setRejectedIds] = useState<Set<string>>(new Set());

  // Clear rejectedIds when pendingChanges updates from the stream
  // (i.e., server state has caught up)
  useEffect(() => {
    setRejectedIds(new Set());
  }, [pendingChanges]);

  const visiblePendingChanges = useMemo(
    () => pendingChanges.filter((c) => !rejectedIds.has(c.id)),
    [pendingChanges, rejectedIds]
  );
